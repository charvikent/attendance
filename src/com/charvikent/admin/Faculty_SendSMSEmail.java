...
@WebServlet("/Faculty_SendSMSEmail")
public class Faculty_SendSMSEmail extends HttpServlet 
{
...
//    int i=0;
	...
	response.setContentType("text/html;charset=UTF-8");
        ...
        try 
        {
            ...
            String sender_id=request.getParameter("fac_id");
            String sent_by=request.getParameter("fac_name");
...
//                            System.out.println(fname);
                            ...
                            {
                            ps=con.prepareStatement("insert into attendance values(null,'"+sid+"','"+student_name+"','"+father_name+"','"+email+"','"+mobile+"','"+message+"','"+morning_session+"','"+afternoon_session+"','"+sender_id+"','"+sent_by+"',now())");
                            i=ps.executeUpdate();
                            }
                            if(absent_session.equals("afternoon"))
                            {
                            	afternoon_session="absent";
                            	ps=con.prepareStatement("update attendance set sid='"+s[r]+"',student_name='"+student_name+"',father_name='"+father_name+"',email='"+email+"',mobile='"+mobile+"',message='"+message+"',afternoon_session='"+afternoon_session+"',sender_id='"+sender_id+"',sent_by='"+sent_by+"',absent_on=now() where sid='"+s[r]+"'");
                                i=ps.executeUpdate();
                            }
...
//                                pw.println("<script src='http://ajax.aspnetcdn.com/ajax/jQuery/jquery-2.1.3.min.js'></script><script>$(document).ready(function(){$.ajax('http://182.18.160.225/index.php/api/bulk-sms?username=RKKIDS&password=RK@kids987&from=RKKIDS&to="+recipient+"&message="+message+"&sms_type=2');});</script>");
                ...
                if(i>0 && notify_type.equals("em") || notify_type.equals("sms") || notify_type.equals("smem"))
                {
                    ...
                    response.sendRedirect("faculty_attendance.jsp");
                }
                else
                {
...
//                        if( (messagedata != null && messagedata !="") || (messagedata.equals(null) && messagedata.equals("")) )
                    ...
                    {
                        ps=con.prepareStatement("insert into notification values(null,'"+sid+"','"+student_name+"','"+father_name+"','"+email+"','"+mobile+"','"+messagedata+"','"+sender_id+"','"+sent_by+"',now())");
                        i=ps.executeUpdate();
                    }
                    else
//                        if( (emessagedata != null && emessagedata !="") || (emessagedata.equals(null) && emessagedata.equals("")) )
                    {
                        ps=con.prepareStatement("insert into notification values(null,'"+sid+"','"+student_name+"','"+father_name+"','"+email+"','"+mobile+"','"+emessagedata+"','"+sender_id+"','"+sent_by+"',now())");
                        i=ps.executeUpdate();
                    }
                ...
                {
                    session.setAttribute("sended", "Successfully Messsage/Email has been Sent");
                    response.sendRedirect("faculty_common_message.jsp");
                }
                else
                {
                    session.setAttribute("error", "Falied to Send the Message");
